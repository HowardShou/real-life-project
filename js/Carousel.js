...
/*  Q5這個問題跟carousel無關，這是我自己寫的jQ遇到的問題:
    
    $(document).ready(function(){
    
    原本想要在全域設定宣告以下變數，指派為jQ物件，以精簡原本的程式碼，
    但是這樣宣告瀏覽器看不懂(ex line:37、42)，我錯在哪?宣告的方式錯了?不能設在全域?
    ...
    $('body').on('click', '#responsive-btn-cart', function() {
      ...
      $('#cart-container').html(nav_others);→本來想寫:Cart.html(nav_others);
      $('#cart-container').addClass('cartshow')  
    });
  
    $('body').on('click', '#cart-container.cartshow', function() {
      $('#cart-container.cartshow').removeClass('cartshow').addClass('carthide');→本來想寫:Showing.removeClass('cartshow').addClass('carthide')
